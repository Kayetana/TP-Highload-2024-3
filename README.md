...
- За год пользователи публикуют ~174 млрд твитов, ~69 млрд изображений и ~5 млрд видео
- Средний размер изображения - ~1.2 Мб, видео - ~40 Мб
- Твиттер доступен для пользователей 17.5 лет

### Размер данных

|    | Таблица            | Размер одной записи | Количество записей за 17.5 лет | Размер всех записей |
|:---|:-------------------|---------------------|--------------------------------|---------------------|
| 1  | User               | ~500 байт           | 1.3 млрд                       | 650 Гб              |
| 2  | User_name_index    | ~24 байта           | 1.3 млрд                       | 30 Гб               |
| 3  | User_aggregated    | ~16 байт            | 1.3 млрд                       | 21 Гб               |
| 4  | User_session       | ~32 байта           | 237.8 млн                      | 8 Гб                |
| 5  | Followings         | ~8 Мб               | 1.3 млрд                       | 10 Пб               |
| 6  | User_actions       | ~8 Мб               | 1.3 млрд                       | 10 Пб               |
| 7  | Embeddings         | ~500 байт           | 1.3 млрд                       | 65 Тб               |
| 8  | Tweet              | ~250 байт           | 3000 млрд                      | 750 Тб              |
| 9  | Tweet_aggregated   | ~24 байта           | 3000 млрд                      | 72 Тб               |
| 10 | Hashtag            | ~8 Мб               | 3 млн                          | 24 Тб               |
| 11 | Hashtag_name_index | ~24 байта           | 3 млн                          | 72 Мб               |
| 12 | Photo_metadata     | ~100 байт           | 1200 млрд                      | 120 Тб              |
| 13 | Video_metadata     | ~120 байт           | 90 млрд                        | 11 Тб               |
| 14 | Photo              | ~1.2 Мб             | 1200 млрд                      | 1440 Пб             |
| 15 | Video              | ~40 Мб              | 87.5 млрд                      | 3500 Пб             |
...
При этом:
- Сервис Auth отвечает за авторизацию
- Сервис User - за регистрацию и подписку пользователей друг на друга
- Сервис Tweet - за публикацию, просмотр и лайк твитов
- Сервис Search - за поиск по юзернейму и хештегу
- Сервис Feed - за получение ленты

| Сервис      | Пиковые значения RPS | Кол-во ядер CPU | RAM      | 
|:------------|----------------------|-----------------|----------|
| Auth        | ~400                 | 4               | 0.4 Гб   |
| User        | ~3 600               | 36              | 4 Гб     |
| Tweet       | ~1 300 000           | 13 000          | 1 300 Гб |
| Search      | ~2 900               | 29              | 3 Гб     |
| Feed        | ~17 900              | 179             | 18 Гб    |


- 1 ядро CPU на 100 запросов 
- 1 Гб RAM на 1000 запросов

Объем диска взят из раздела [Размер данных](#размер-данных).

| Хранилище     | Объем диска | 
|:--------------|-------------|
| Elasticsearch | 30 Гб       |
| Aerospike     | 10 Пб       |
| Cassandra     | 900 Тб      |
...
| Сервис        | Хостинг     | Конфигурация                                                   | Покупка 1 шт [[13]](https://www.broadberry.eu/rackmount-servers) | Количество машин | 
|:--------------|-------------|----------------------------------------------------------------|------------------------------------------------------------------|------------------|
| k8s node      | Собственный | 64 CPU Cores / 8x32 GB RAM / 2x250 GB SSD / 2x1 Gb/s LAN ports | 11 330 $                                                         |                  |
| Elasticsearch | Собственный | 8 CPU Cores / 2×8 GB RAM / 2x1.6 TB SSD / 2x1 Gb/s LAN ports   | 7 900 $                                                          |                  |
| Aerospike     | Собственный | 8 CPU Cores / 2×8 GB RAM / 2x1.6 TB SSD / 2x1 Gb/s LAN ports   | 7 900 $                                                          |                  |
| Cassandra     | Собственный | 8 CPU Cores / 2×8 GB RAM / 2x1.6 TB SSD / 2x1 Gb/s LAN ports   | 7 900 $                                                          |                  |
| ClickHouse    | Собственный | 8 CPU Cores / 2×8 GB RAM / 2x1.6 TB SSD / 2x1 Gb/s LAN ports   | 7 900 $                                                          |                  |
| FlockDB       | Собственный | 8 CPU Cores / 2×8 GB RAM / 2x1.6 TB SSD / 2x1 Gb/s LAN ports   | 7 900 $                                                          |                  |
| Chroma        | Собственный | 8 CPU Cores / 2×8 GB RAM / 2x250 GB SSD / 2x1 Gb/s LAN ports   | 6 500 $                                                          |                  |
| Redis         | Собственный | 8 CPU Cores / 2×8 GB RAM / 2x250 GB SSD / 2x1 Gb/s LAN ports   | 6 500 $                                                          |                  |
