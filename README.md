...
Когда пользователь выполняет поиск и вводит несколько символов, необходимо выполнить
префиксный поиск и вернуть ему несколько вариантов имеющихся имен или хэштегов,
которые начинаются с заданных символов.

Для быстрого поиска по префиксам создается индекс по текстовому полю с параметром index_prefixes.
Каждое слово разбивается на набор префиксов, которые затем сохраняются в инвертированном индексе.
...
- ```Разнообразие авторов``` - избегание длинных последовательностей твитов одного автора
- ```Баланс контента``` - обеспечение баланса In-Network и Out-of-Network твитов,
их должно быть примерно поровну
- ```Учет негативного фидбека``` - снижение оценки твитов, похожих на те,
где пользователь дал отрицательную обратную связь
- ```Фильтрация по видимости``` - фильтрация твитов от заблокированных аккаунтов,
