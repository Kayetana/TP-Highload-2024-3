...
Рассмотрим, как работает алгоритм для выполнения одного запроса на получение ленты. Алгоритм состоит из трех основных этапов.
...
| Go            | Основной язык для backend-сервисов                                              | Простота и минимализм, высокая производительность, надежность                                                                                      |
...
| Kafka         | Распределенный брокер сообщений                                                 | Эффективная обработка сообщений в режиме реального времени, партиционирование, масштабируемость и отказоустойчивость                               |
| Aerospike     | Хранений действий пользователей                                                 | Автоматическое масштабирование, SSD optimized, миллион операций чтения/записи в секунду                                                            |
| Cassandra     | Хранение пользователей, твитов, хэштегов и метаданных медиафайлов               | Автоматическое масштабирование, гибкая схема данных, основанная на column-families, поддержка ACID                                                 |
| ClickHouse    | Хранение агрегированных данных пользователей и твитов, которые часто изменяются | Обработка больших объемов данных с высокой скоростью                                                                                               |
| Elasticsearch | Индексы для поиска пользователей по имени и твитов по хэштегам                  | Высокая производительность поиска                                                                                                                  |
| FlockDB       | Хранение информации о подписках в виде списков смежности                        | Высокая скорость операций добавления, обновления и удаления; выдерживает 20 000 операций записи в секунду и 100 000 операций чтения в секунду      |
| Redis         | Хранение пользовательских сессий                                                | Простота использования                                                                                                                             |
| Amazon S3     | Хранение медиафайлов                                                            | Надежность, масштабируемость, отказоустойчивость                                                                                                   |
| Memcached     | Кэширование                                                                     | Простота, гибкость настройки                                                                                                                       |
