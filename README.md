...
- Сервис Model Training периодически переобучает модели, хранящиеся в S3 Models
...
- Сервис User - за регистрацию и получение информации о пользователе
- Сервис Subscribers - за подписку пользователей друг на друга
...
|:------------|----------------------|-----------------|----------|-----------------------------------------|
| User        | ~1 000               | 10              | 1 Гб     | 1 * 5 ДЦ = 5                            |
| Search      | ~2 900               | 29              | 3 Гб     | 1 * 5 ДЦ = 5                            |
| Subscribers | ~3 600               | 36              | 4 Гб     | 1 * 5 ДЦ = 5                            |
...
- При расчете количества машин необходимо учитывать датацентры и коэффициент репликации
- Будем считать, что срок использования одной машины - 5 лет. Тогда:  
```Ежемесячная сумма амортизации = Стоимость машины / 5 лет * 12 мес```

| Сервис        | Хостинг     | Конфигурация                               | Кол-во машин         | Покупка 1 шт на 5 лет | Ежемесячная сумма амортизации | Аренда 1 шт в месяц | Аренда 1 шт за 5 лет | 
|:--------------|-------------|--------------------------------------------|----------------------|-----------------------|-------------------------------|---------------------|----------------------|
| k8s node      | Собственный | 64 CPU Cores / 4×16 GB RAM / 2x240 GB SSD  | 1 330                | 11 800 $              | 200 $                         | 400 $               | 24 000 $             |
| Redis         | Собственный | 16 CPU Cores / 4×16 GB RAM / 2x240 GB SSD  | 1 * 2 RF * 5 ДЦ = 10 | 6 500 $               | 110 $                         | 340 $               | 20 400 $             |
| Chroma        | Собственный | 16 CPU Cores / 4×16 GB RAM / 2x240 GB SSD  | 1 * 5 ДЦ = 5         | 6 500 $               | 110 $                         | 340 $               | 20 400 $             |
| Elasticsearch | Собственный | 16 CPU Cores / 4×64 GB RAM / 2x960 GB SSD  | 1 * 5 ДЦ = 5         | 7 800 $               | 130 $                         | 180 $               | 10 800 $             |
| ClickHouse    | Собственный | 16 CPU Cores / 4×64 GB RAM / 4x1.9 TB SSD  | 1 * 3 RF * 5 ДЦ = 15 | 9 000 $               | 150 $                         | 180 $               | 10 800 $             |
| Cassandra     | Собственный | 16 CPU Cores / 4×64 GB RAM / 4x7.6 TB SSD  | 2 * 3 RF * 5 ДЦ = 30 | 11 000 $              | 180 $                         | 260 $               | 15 600 $             |
| Aerospike     | Собственный | 16 CPU Cores / 4×64 GB RAM / 10x7.6 TB SSD | 8 * 2 RF * 5 ДЦ = 80 | 17 000 $              | 280 $                         | 340 $               | 20 400 $             |
| FlockDB       | Собственный | 16 CPU Cores / 4×64 GB RAM / 10x7.6 TB SSD | 8 * 5 ДЦ = 40        | 17 000 $              | 280 $                         | 340 $               | 20 400 $             |
| S3            | Amazon      | 280 Пб                                     | -                    | -                     | -                             | -                   | 68 100 000 $         |


По сумме амортизации можно сделать вывод, что покупка машин выгоднее чем аренда.
Посчитаем полную стоимость покупки машин.

```
1330 * 11800 + 10 * 6500 + 5 * 6500 + 5 * 7800 + 15 * 9000 + 30 * 11000 + 80 * 17000 + 40 * 17000 = 18 335 500  
Стоимость покупки = ~18 млн $
```

```
Стоимость всех ресурсов на 5 лет: 18 млн $ + 68 млн $ (Amazon S3) = ~86 млн $
```
