...
таких как оскорбления и названия наркотических веществ.

Это полезно для выявления открытой агрессии, например "Я ненавижу [запрещенное слово]".
Однако контент может не содержать слов из списка, но тем не менее нести угрозу для пользователей.
Поэтому необходима также более гибкая модерация на основе классификаторов.

#### Классификация текста

XLM-RoBERTa (XLM-R) - модель NLP, предварительно обученная на данных на 100 языках общим объёмом 2 ТБ.  
Ключевым преимуществом многоязычных моделей является их способность передавать знания между языками.
...
Среди CNN может быть выбрана DenseNet. В архитектуре DenseNet каждый уровень получает входные данные от предыдущих уровней
и передает свою собственную информацию всем последующим уровням. Это означает что конечный уровень будет содержать 
информацию от каждого отдельного уровня, включая самый первый. Это позволяет сети более эффективно получать знания
...
- Сервис User Action Worker читает это с помощью двух консьюмер-групп.
Затем параллельно один консьюмер записывает действие пользователя в Aerospike,
...
| Сервис        | Хостинг     | Конфигурация                                                   | Покупка 1 шт | 
|:--------------|-------------|----------------------------------------------------------------|--------------|
