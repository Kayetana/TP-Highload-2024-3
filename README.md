...
Kubernetes выполняет service discovery, масштабирование через auto-scaling,
распределение сервисов по узлам и их перезапуск при падении.  
Также Kubernetes удаляет зависшие приложения, которые перестают отвечать на healthcheck`и.
